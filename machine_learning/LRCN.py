...
val_gen = utils.image_batch_generator(val_directory, lb, batch_size, input_shape, n_classes, mode = "eval")

# predict new labels on the test set
predIdxs = model.predict(x=val_gen, steps=(val_count//batch_size))
predIdxs = np.argmax(predIdxs, axis=1)

# get the true labels of the test set
val_set = utils.create_image_sets(val_directory)
    ...
    if n > 2 and n % seq_len >= 0:
        for i in range(0, n % seq_len + 1):
            ...
            if utils.is_valid_sequence(sequence, seq_len):
                label = utils.get_classname(val_directory, sequence[0])
...
difference = len(val_labels)-len(predIdxs)


# print the number of available sets per class category
for classname in labels:
    directory_name = val_directory + "/" + classname
    sequence_counts = utils.get_valid_sequence_count(utils.create_image_sets(directory_name), 3)
...
print(classification_report(val_labels[:-difference], predIdxs,
	target_names=lb.classes_))

print(" ")
print("Confusion Matrix") 
print(confusion_matrix(val_labels[:-difference], predIdxs))
